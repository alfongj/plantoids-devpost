(function (global, factory) {
  ...
  class LSystem {
    ...
    getProductionResult(p, index, part, params, recursive = false) {
      ...
      if (precheck === false) {
        result = false;
          ...
          if (p.isStochastic) {
            threshWeight = this.rng() * p.weightSum;
            currentWeight = 0;
          }
          /*
    ...
    /**
     *
     * @param {Number} n Number of iterations to apply
     * @param {=Function} rng Function that generates a random number
     * @returns lastIteration
     */
    iterate(n = 1, rng) {
      if (rng) {
        this.rng = rng;
